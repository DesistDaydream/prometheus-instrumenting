# 构建
```shell
docker build -f console_agent_exporter/Dockerfile -t console-agent-exporter:v0.5.0-proxy .
```
...
## 测试
```shell
go run console_agent_exporter/main.go --web.listen-address=':9122' --console-agent-server='http://gdas-proxy.unicom.cqcq.ehualu.it:38443' --log-level=debug 
```
