...
func (ScrapeCluster) Help() string {
	return "Gdas Cluster Info"
}

// Scrape 从客户端采集数据，并将其作为 Metric 通过 channel(通道) 发送。主要就是采集 Gdas 集群信息的具体行为。
// 该方法用于为 ScrapeCluster 结构体实现 Scraper 接口
func (ScrapeCluster) Scrape(client *GdasClient, ch chan<- prometheus.Metric) (err error) {
	// 声明需要绑定的 响应体 与 结构体
	var (
		respBody []byte
		data     Magazines
	...
	ch <- prometheus.MustNewConstMetric(cluster, prometheus.GaugeValue, undistributedCount, "undistributedCount")
	return nil
}
